<div class="portfolio-content portfolio-post-<?php echo esc_attr($showcase_id); ?> main-container porto-full-image-style<?php if ($portfolio_link_post == 'lightbox') { ?> lightbox-parent<?php } ?>">
	...
	if(in_array('advanced-masonry-portfolio-builder/advanced-masonry-portfolio-builder.php', apply_filters('active_plugins', get_option('active_plugins')))){
		...
		ampb_ta_filter_masonry_temp($use_filter, $portfolio_showcase_cats, $portfolio_select_display_post);
	} ?>
	<div class="portfolio-block-wrap grid-template clearfix">
		...
		if (has_post_thumbnail()) {
			...
			$image_portores =  aq_resize($img_url_porto[0],  $portfolio_width_image , '', true, true, true);
...
if(in_array('advanced-masonry-portfolio-builder/advanced-masonry-portfolio-builder.php', apply_filters('active_plugins', get_option('active_plugins')))){
	wp_enqueue_script( 'ampb-advanced-masonry-portfolio-builder-isotope', ADVANCED_MASONRY_PORTFOLIO_BUILDER_URL . 'assets/js/isotope.min.js', array('jquery', 'imagesloaded'), '', false );
	wp_enqueue_script( 'ampb-advanced-masonry-portfolio-builder-infinite-scroll',  ADVANCED_MASONRY_PORTFOLIO_BUILDER_URL . 'assets/js/infinite-scroll.min.js', array('masonry'), '', false );

	if ($portfolio_link_post == 'lightbox') {
		wp_enqueue_style( 'ampb-advanced-masonry-portfolio-builder-light-gallery', ADVANCED_MASONRY_PORTFOLIO_BUILDER_URL . 'assets/css/lightgallery.css', array(), '', 'all' );
		wp_enqueue_script( 'ampb-advanced-masonry-portfolio-builder-light-gallery',  ADVANCED_MASONRY_PORTFOLIO_BUILDER_URL . 'assets/js/lightgallery.js', array('jquery'), '', false );
	}
}
else {
...
wp_enqueue_script( 'ta-portfolio-awesome-effect-grid', plugin_dir_url('README.txt') . PORTFOLIO_AWESOME_NAME . '/public/js/effect-grid.js', array('jquery'), '', false ); ?>
...
if(class_exists('Elementor\Plugin')) {
	...
	<?php 
	if(in_array('advanced-masonry-portfolio-builder/advanced-masonry-portfolio-builder.php', apply_filters('active_plugins', get_option('active_plugins')))){
		ampb_ta_filter_masonry_script($showcase_id, $use_filter, $portfolio_pagination_type);
	} ?>
	<?php }
} else { ?>
	<script>
