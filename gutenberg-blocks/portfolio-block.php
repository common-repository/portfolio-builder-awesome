...
<?php
global $post;
$ta_ids = $fields['portfolio_gutenberg_block'][0]['id'];
...
wp_enqueue_style( 'ta-portfolio-awesome', plugin_dir_url('README.txt') . PORTFOLIO_AWESOME_NAME . '/public/css/portfolio-awesome-public.css', array(), '1.0.0', 'all' );
...
$portfolio_select_display_post = carbon_get_post_meta( $showcase_id, 'portfolio_select_display_post' );
$portfolio_awesome_showcase_posts = carbon_get_post_meta( $showcase_id, 'portfolio_showcase_posts' );
$portfolio_showcase_cats = carbon_get_post_meta( $showcase_id, 'portfolio_showcase_cats' );
$portfolio_showcase_order = carbon_get_post_meta( $showcase_id, 'portfolio_showcase_order' );
$portfolio_showcase_order_by = carbon_get_post_meta( $showcase_id, 'portfolio_showcase_order_by' );

// OPTION LAYOUT

$portfolio_style_main = carbon_get_post_meta( $showcase_id, 'portfolio_awesome_showcase_style_main' );
if($portfolio_style_main == 'grid') {
    $portfolio_style = carbon_get_post_meta( $showcase_id, 'portfolio_awesome_showcase_style' );
}
elseif($portfolio_style_main == 'masonry') {
    $portfolio_style = carbon_get_post_meta( $showcase_id, 'portfolio_awesome_showcase_style2' );
}
elseif($portfolio_style_main == 'carousel') {
    $portfolio_style = carbon_get_post_meta( $showcase_id, 'portfolio_awesome_showcase_style3' );
}
elseif($portfolio_style_main == 'justified') {
    $portfolio_style = carbon_get_post_meta( $showcase_id, 'portfolio_awesome_showcase_style4' );
}
elseif($portfolio_style_main == 'slider') {
    $portfolio_style = carbon_get_post_meta( $showcase_id, 'portfolio_awesome_showcase_style5' );
}

$portfolio_awesome_post_hover = carbon_get_post_meta( $showcase_id, 'portfolio_showcase_hover' );
$portfolio_awesome_post_loading_grid = carbon_get_post_meta( $showcase_id, 'portfolio_showcase_loading_grid' );
$portfolio_awesome_post_per_page = carbon_get_post_meta( $showcase_id, 'portfolio_showcase_items' );
$portfolio_showcase_choose_grid = carbon_get_post_meta( $showcase_id, 'portfolio_showcase_choose_grid' );

$portfolio_showcase_choose_column = carbon_get_post_meta( $showcase_id, 'portfolio_showcase_choose_column' );
$portfolio_showcase_choose_column_tablet = carbon_get_post_meta( $showcase_id, 'portfolio_showcase_choose_column_tablet' );
$portfolio_showcase_choose_column_mobile = carbon_get_post_meta( $showcase_id, 'portfolio_showcase_choose_column_mobile' );

$portfolio_column_carousel = carbon_get_post_meta( get_the_ID(), 'portfolio_showcase_choose_column_carousel' );

$portfolio_use_arrow = carbon_get_post_meta( $showcase_id, 'portfolio_use_arrow' );
$portfolio_select_arrow = carbon_get_post_meta( $showcase_id, 'portfolio_select_arrow' );
$portfolio_select_dot = carbon_get_post_meta( $showcase_id, 'portfolio_select_dot' );
$portfolio_use_pagination = carbon_get_post_meta( $showcase_id, 'portfolio_use_pagination' );

// IMAGE SETTING

$portfolio_width_image = carbon_get_post_meta( $showcase_id, 'portfolio_width_image' );
$portfolio_height_image = carbon_get_post_meta( $showcase_id, 'portfolio_height_image' );
$portfolio_use_crop = carbon_get_post_meta( $showcase_id, 'portfolio_use_crop' );
            
// LINK GOES TO
$portfolio_link_post = carbon_get_post_meta( $showcase_id, 'portfolio_select_link_post' );

// PAGINATION & FILTER
$portfolio_text_load_more = carbon_get_post_meta( $showcase_id, 'portfolio_text_load_more' );
$portfolio_pagination_type = carbon_get_post_meta( $showcase_id, 'portfolio_pagination_type' );
$use_filter = carbon_get_post_meta( $showcase_id, 'portfolio_use_filter' );
$portfolio_showcase_category_filter = carbon_get_post_meta( $showcase_id, 'portfolio_showcase_category_filter' );

$portfolio_column_carousel = carbon_get_post_meta( $showcase_id, 'portfolio_showcase_choose_column_carousel' );
...
// The Loop
if ( $ta_portfo->have_posts() && $ta_ids != '' ) {
